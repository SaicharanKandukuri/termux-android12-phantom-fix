...
cat <<- EOF >> ~/.bashrc
cur_phantom_peak=\$(adb -s localhost:55557 shell device_config get activity_manager max_phantom_processes)
...
if [ "\$(adb -s localhost:55557 shell device_config get activity_manager max_phantom_processes)" == 'null' ]; then
